@section('title', 'Admin - Tambah Belanja')
                    ...
                    <form action="{{ route('admin.produk-umkm.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <x-input id="nama_produk" class="block mt-1 w-full p-2" type="text" name="nama_produk" :value="old('nama_produk')" required autofocus placeholder="Contoh: Keripik Singkong Balado" />
                            </div>

                            <!-- Harga -->
                            <div>
                                <x-label for="format_harga" value="{{ __('Harga (Rp)') }}" />
                                <x-input id="format_harga" class="block mt-1 w-full p-2" type="text" name="format_harga" placeholder="Contoh: Rp 15.000/250 gram atau Rp 3.000/pcs (isi 10 buah)"/>
                            </div>

                            <!-- Deskripsi -->
                            <div>
                                <x-label for="deskripsi" value="{{ __('Deskripsi Singkat') }}" />
                                <textarea id="deskripsi" name="deskripsi" rows="4" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm p-2" required placeholder="Jelaskan produk secara singkat...">{{ old('deskripsi') }}</textarea>
                            </div>

                            <!-- Nomor WhatsApp -->
                            <div>
                                <x-label for="nomor_wa" value="{{ __('Nomor WhatsApp Penjual') }}" />
                                <x-input id="nomor_wa" class="block mt-1 w-full p-2" type="text" name="nomor_wa" placeholder="Contoh: 6281234567890 (tanpa +)" />
                            </div>
                            ...
                            <div class="flex items-center justify-end mt-4">
                                <a href="{{ route('admin.produk-umkm.index') }}" class="text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white underline mr-4 p-2">
                                    Batal
                                </a>
                                <x-button>
                                    {{ __('Simpan Produk') }}
